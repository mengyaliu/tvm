...
input_data = np.fromfile("meta_info/resnet50/data/resnet50_input_data.bin", dtype='float32').reshape(intput_shape)
input = tvm.nd.array(input_data, tvm.cpu(0))

# upload weight data to device
weight_data = np.fromfile("meta_info/resnet50/resnet50_weight.bin", dtype='float32')
...
# load Loadable
resnet50 = tvm.module.load("meta_info/resnet50/resnet50.tvm_meta.json")
